...
impl Frog {
    ...
    pub fn hop(&mut self) {
        ...
        info!(target: "Frog::hop", "Frog has hopped and has {} energy left", self.energy);
        if self.energy == 0 {
            // 4. Use warn!() to warn that the frog will go to sleep since he ran out of energy
            warn!(target: "Frog::hop", "Frog has run out of energy, will now go to sleep");
            self.sleep();
        }
    }
...
impl Default for Frog {
    fn default() -> Self {
        ...
        trace!(target: "Frog::default", "Default value for Frog = {:?}", frog);
